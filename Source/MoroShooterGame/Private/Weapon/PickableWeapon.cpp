...
void APickableWeapon::AnnounceAbleToPickupOnAll_Implementation()
{
	...
	GetOverlappingActors(OverlappedActors, UPickableSlot::StaticClass());
	for (AActor* Target : OverlappedActors)
	{
		// This is guaranteed, but still, more check, more safe.
		if (Target->GetClass()->ImplementsInterface(UPickableSlot::StaticClass()))
		{
			IPickableSlot* PickableSlot = Cast<IPickableSlot>(Target);
			PickableSlot->Execute_OnAbleToPickupItem(Target, this);
		}
	}
}
...
void APickableWeapon::AnnounceLostTrackOnAll_Implementation()
{
	...
	for (AActor* Target : OverlappedActors)
	{
		...
		{
			IPickableSlot* PickableSlot = Cast<IPickableSlot>(Target);
			PickableSlot->Execute_OnLostTrackOfItem(Target, this);
		}
	}
}
...
{
	UE_LOG(LogTemp, Warning, TEXT("Attach Overlap"));
	if (Execute_IsOpenForPickup(this) && OtherActor->GetClass()->ImplementsInterface(UPickableSlot::StaticClass()))
	{
		IPickableSlot* PickableSlot = Cast<IPickableSlot>(OtherActor);
		PickableSlot->Execute_OnAbleToPickupItem(OtherActor, this);	// Why? what happens if ues OnAbleToPickupItem directly?
	}
}
	...
	AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	...
	{
		IPickableSlot* PickableSlot = Cast<IPickableSlot>(OtherActor);
		PickableSlot->Execute_OnLostTrackOfItem(OtherActor, this);		// Why? what happens if ues OnLostTrackOfItem directly?
	}
}
